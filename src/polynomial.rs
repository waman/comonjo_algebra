...
pub enum Polynomial<C> {
    Zero(),
    Constant(C),
    Dense(Vec<C>),
    Spares(HashMap<usize, C>)
}

impl<C> Polynomial<C> {

    pub fn zero<D>() -> Polynomial<D> {
        Polynomial::Zero()
    }

    pub fn constant<D: Zero>(c: D) -> Polynomial<D> {
        if c.is_zero() {
            Polynomial::Zero()
        } else {
    ...
    // pub fn linear<D: Zero>(c: D) -> Polynomial<D> {
    //     Polynomial::spears::<D, 1>([(1, c)])
    // }
    ...
    pub fn dense_from_vec<D: Zero>(mut coeffs: Vec<D>) -> Polynomial<D> {
    ...
    pub fn spears_from_map<D: Zero>(mut coeffs: HashMap<usize, D>) -> Polynomial<D> {
    ...
    pub fn degree(&self) -> usize {
        match self {
            ...
            Polynomial::Dense(vec) => vec.len() - 1,
            Polynomial::Spares(map) => *map.keys().max().unwrap(), 
        }
    }
}
...
macro_rules! spears {
    [ $t:ty; $( $x:expr ),* ] => {
        Polynomial::<$t>::spears_from_map(std::collections::HashMap::from([ $( $x ),* ]))
    }
}
 
impl<C> Display for Polynomial<C> 
        where C: Display + Zero + One + PartialEq + Neg<Output=C>{

    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {

        match self {
            Polynomial::Zero() => f.write_str("0"),
            Polynomial::Constant(c) => f.write_fmt(format_args!("{}", c)),

            Polynomial::Dense(vec) => {
                let v: Vec<(usize, &C)> = vec.iter().enumerate().filter(|c| !c.1.is_zero()).collect();
                display_polynomial(f, v)
            },

            Polynomial::Spares(map) => {
                let mut v: Vec<(usize, &C)> = map.iter().map(|e| (*e.0, e.1)).collect();
                v.sort_by_key(|k| k.0);
                display_polynomial(f, v)
            },
        }
...
fn display_polynomial<C: Zero + Display>(f: &mut std::fmt::Formatter<'_>, coeffs: Vec<(usize, &C)>) -> std::fmt::Result {
    ...
    for (i, c) in coeffs {
        ...
        result = if is_first {
            match i {
                0 => f.write_fmt(format_args!("{}", c)),
                1 => f.write_fmt(format_args!("{}x", c)),
                _ => f.write_fmt(format_args!("{}x^{}", c, i)),
            }
        } else {
            let s = match i {
                0 => format!("{:+}", c),
            ...
            let ss = s.replace("+1x", "+x").replace("-1x", "-x");
            f.write_str(&ss)
        };
        is_first = false;
    }
